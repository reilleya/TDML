...
namespace TDML
{
	void terrain::setHeightMap(std::vector<std::vector<float>> newheightmap)
		...
		void terrain::setScale(int xz, int y)
		{
			scalexz = xz;
			scaley = y;
		}

		int terrain::getScaleXZ()
		{
			return scalexz;
		}

		int terrain::getScaleY()
		{
			return scaley;
		}
		...
		float terrain::getHeightMapAt(float x, float z)
		{
			...
			nx*=dimensions;
			nz*=dimensions;
			float rnx = Math.simpleRoundDown(nx);
			float rnz = Math.simpleRoundDown(nz);
			float height = 0;
			float parX = x - rnx;
			float parY = z - rnz;
			if ((parX + parY) < 1)
			{
				...
				height += (heightmap[nx+1][nz] - heightmap[nx][nz]) * parX;
				height += (heightmap[nx][nz+1] - heightmap[nx][nz]) * parY;
			} 
			else 
			{
				height = heightmap[nx+1][nz+1];
				height += (heightmap[nx+1][nz] - heightmap[nx+1][nz+1]) * ( 1.0f - parY);
				height += (heightmap[nx][nz+1] - heightmap[nx+1][nz+1]) * ( 1.0f - parX);
			}
			height -= 128;
			//height *= scaley;
			return height;
		...
		void terrain::generateVBO()
		{
			...
			for(int v1 = 0; v1 < dimensions-1; v1++)
			{
				...
				for(int v2 = 0; v2 < dimensions-1; v2++)
				{
					...
					//cout << "\t\t" << points[v1][v2].getX() << " " << points[v1][v2].getY() << " " << points[v1][v2].getZ()<<endl;
					normals[(p*18)] = 0;
					normals[(p*18)+1] = 1;
					normals[(p*18)+2] = 0;
					coords[(p*12)] = 0;
					...
					//cout << "\t\t" << points[v1+1][v2].getX() << " " << points[v1+1][v2].getY() << " " << points[v1+1][v2].getZ()<<endl;
					normals[(p*18)+3] = 0;
					normals[(p*18)+4] = 1;
					normals[(p*18)+5] = 0;
					coords[(p*12)+2] = 0;
					...
					//cout << "\t\t" << points[v1][v2+1].getX() << " " << points[v1][v2+1].getY() << " " << points[v1][v2+1].getZ()<<endl;
					normals[(p*18)+6] = 0;
					normals[(p*18)+7] = 1;
					normals[(p*18)+8] = 0;
					coords[(p*12)+4] = 1;
					...
					geometry[(p*18)+14] = points[v1][v2+1].getZ();//Something screwed up here, always 0...
					//cout << "\t\t" << points[v1+1][v2+1].getX() << " " << points[v1+1][v2+1].getY() << " " << points[v1+1][v2+1].getZ()<<endl;
					normals[(p*18)+12] = 0;
					normals[(p*18)+13] = 1;
					normals[(p*18)+14] = 0;
					coords[(p*12)+8] = 0;
					...
					//cout << "\t\t" << points[v1][v2+1].getX() << " " << points[v1][v2].getY() << " " << points[v1][v2+1].getZ()<<endl;
					normals[(p*18)+15] = 0;
					normals[(p*18)+16] = 1;
					normals[(p*18)+17] = 0;
					coords[(p*12)+10] = 1;
